...
Provide one concrete maintainer workflow for validating repo state, package contents, and install guidance before publishing `cdp-browser`.
...
`prepublishOnly` intentionally does **not** run the standalone host-only `npm run verify -- startup-profile`, `npm run verify -- real-upstream`, `npm run verify -- dogfood` modes; those remain separate `npm run verify` modes in [`scripts/project.mjs`](https://github.com/fitchmultz/cdp-browser/blob/main/scripts/project.mjs). The platform matrix includes its own fast target-local build/package gate and browser dogfood suite, and is automated through the `release` slice.
...
1. Open the URL with the native `cdp_browser` tool in a fresh session.
...
Use this validation prompt after changing click enrichment, tab pinning, ref preflight, form-fill batching, artifact handling, recording, or prompt guidance. It is intentionally more stateful than `example.com` and uses a natural user-style request so the transcript shows what the agent chooses on its own. Do **not** mention `cdp_browser`, snapshots, refs, `batch`, `eval`, or upstream command names in the prompt; those are evaluator expectations, not user instructions.
...
```bash
pi --approve --no-extensions --no-skills -e . --model openai-codex/gpt-5.5:minimal --tools cdp_browser --session-dir "$SESSION_DIR"
```
...
- `cdp_browser` is registered without requiring optional Brave config
- any optional companion tools remain governed by their own configuration gates
- the registered `cdp_browser` source resolves inside the extracted packed package path, not the working checkout
- the packaged `cdp_browser` tool can be executed through Pi's loaded native tool definition with a deterministic fake upstream `agent-browser --version` binary
...
5. Run a smoke prompt that exercises `cdp_browser`.
...
| **Real-browser cleanup** | `real-upstream`, Sauce Demo, and live-site runs can leave defunct Chrome/`agent-browser` children if a session aborts mid-flow. | Close via `cdp_browser` / `agent-browser` `close`, kill stray tmux sessions, and remove temp screenshots/HARs under `/tmp` or your chosen artifact dirs. |
| **Automated prompt driving** | Grepping tmux pane text for words that also appear in the **user** prompt (`PASS`, `FAIL`, `checkout overview`, `Smoke result:`) can false-complete before the agent finishes. | Wait for pane idle (no `Working…`), `cdp_browser close` / `Artifact lifecycle`, or JSONL tool results—not instruction phrases copied from the prompt. |
...
That npm script sets `PI_CDP_BROWSER_REAL_UPSTREAM=1` for the test process. To run `test/agent-browser.real-upstream-contract.test.ts` directly (for example with `node --test` and `tsx`), set the same variable yourself; the suite is skipped when it is unset.
...
- **Recording FPS paths and pinning:** the opt-in `test/agent-browser.batch-fidelity.test.ts` native case checks direct/raw/stdin preflight before dispatch, actual start/restart destinations and pinned targets with leading `--fps`, conservative start-ref protection, FPS-only restart continuity, and valid 12-fps WebM output after explicitly logged 12-second fixture captures. Run it with `PI_CDP_BROWSER_REAL_UPSTREAM=1`; it requires native 0.37 or newer. Keep focused older-native controls for fresh-page starts and the start-then-ref batch latch when changing that policy.
...
```text
Use the cdp_browser tool to open https://react.dev and then take an interactive snapshot.
```
...
```bash
npm exec --package cdp-browser -- cdp-browser-doctor
npm run verify -- release
pi --no-extensions -e npm:cdp-browser@<version>
```
