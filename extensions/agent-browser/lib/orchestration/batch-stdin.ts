...
function validateUserBatchStep(step: unknown, index: number): { error: string; ok: false } | { ok: true; step: BatchCommandStep } {
	if (!Array.isArray(step)) {
		return {
			error: `cdp_browser batch stdin step ${index} must be a non-empty array of string command tokens.${BATCH_STDIN_EXAMPLE}`,
			ok: false,
		};
	}
	if (step.length === 0) {
		return {
			error: `cdp_browser batch stdin step ${index} must not be empty.${BATCH_STDIN_EXAMPLE}`,
			ok: false,
		};
	}
	...
	if (invalidTokenIndex !== -1) {
		return {
			error: `cdp_browser batch stdin step ${index} token ${invalidTokenIndex} must be a string.${BATCH_STDIN_EXAMPLE}`,
			ok: false,
		};
	}
...
export function parseBatchStdinJsonArray(stdin: string | undefined): { error?: string; steps?: unknown[] } {
	...
	try {
		...
		if (!Array.isArray(parsed)) {
			return { error: `cdp_browser batch stdin must be a JSON array of command steps.${BATCH_STDIN_EXAMPLE}` };
		}
		return { steps: parsed };
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		return { error: `cdp_browser batch stdin could not be parsed as JSON: ${message}.${BATCH_STDIN_EXAMPLE}` };
	}
}
