...
test("inspectManagedSessionDaemon waits through a temporarily busy daemon", { concurrency: false, timeout: 15_000 }, async () => {
	const tempDir = await mkdtemp(join(tmpdir(), "cdp-browser-daemon-policy-"));
...
`);
	...
	try {
		const result = await withPatchedEnv({
			...
			PI_CDP_BROWSER_PROCESS_TIMEOUT_MS: "50",
			PI_CDP_BROWSER_TEST_CUSTOM_SESSION_INFO: "1",
		}, () => inspectManagedSessionDaemon({ cwd: tempDir, sessionName: "cdpb-slow" }));
